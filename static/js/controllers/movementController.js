...
export function attachMovement({ renderer, getSnapshot, roomCode, playerId }) {
  let cooldownAt = 0;
  const COOLDOWN = 70;
  let inputLocked = false;

  function setLocked(v) { inputLocked = v; }
  ...
  function inBounds(s, x, y) { return x >= 0 && y >= 0 && x < s.size && y < s.size; }
  function occupied(s, x, y, myRole) {
    const other = myRole === 'R' ? 'B' : 'R';
    const op = s.positions[other];
    return op && op[0] === x && op[1] === y;
  }
  function isStar(s, x, y) { return s.target && s.target[0] === x && s.target[1] === y; }
  function capturerIsMe(s) { return s.capturer === s.myRole; }
  ...
  function tryMove(dx, dy) {
    ...
    const nx = mePos[0] + dx, ny = mePos[1] + dy;
    if (!inBounds(s, nx, ny)) return;
    if (occupied(s, nx, ny, s.myRole)) return;
    if (isStar(s, nx, ny) && !capturerIsMe(s)) return;

    // --- optimistic local update (just the dot)
    renderer.updatePlayer(s.myRole, [nx, ny]);
