...
document.addEventListener("DOMContentLoaded", function () {
      ...
      .then((response) => {
        ...
        if (!response.ok) {
          ...
          return response
            ...
            .catch(() => {
              throw new Error(
                "Server returned an error response: " + response.status
              );
            });
        }
  ...
  function connectToSocketIO(roomCode, playerId, isModerator = false) {
    debug(
      `Connecting to Socket.IO, room: ${roomCode}, player: ${playerId}, moderator: ${isModerator}`
    );
    ...
    socket.on(EVT.ROOM_STATE, function (data) {
      ...
      if (
        Object.keys(players).filter((pid) => !players[pid].is_moderator)
          .length >= 2 &&
        isModerator
      ) {
    ...
    socket.on(EVT.PLAYER_JOINED, function (data) {
      ...
      const existingPlayer = Array.from(playersList.children).find(
        (li) => li.dataset.playerId === data.player_id
      );
  ...
  async function copyTextSafe(text) {
    ...
    ta.select();
    document.execCommand("copy"); // deprecated but fine as fallback
